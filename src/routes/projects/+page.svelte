<script lang="ts">
	...
	let activeDomain = $state('Featured Projects');
	const domains = [
		'Generative AI',
		'Software Engr',
		'Machine Learning',
...
<div class="relative flex flex-col gap-8 md:flex-row">
	...
	<div class="mb-4 flex w-full justify-center md:hidden">
		<button
		...
		{#if showMobileMenu}
			...
			>
				<button
					onclick={() => {
						activeDomain = 'Featured Projects';
						showMobileMenu = false;
					}}
					class="text-left font-semibold">Featured projects</button
				>
				...
				{#each domains as domain (domain)}
					<button
						...
						class="pl-4 text-left transition-colors hover:text-white/70">{domain}</button
					>
				{/each}
			</div>
		{/if}
	...
	>
		<button
			onclick={() => (activeDomain = 'Featured Projects')}
			class="mb-6 text-left text-xl font-bold {activeDomain === 'Featured Projects'
				? 'text-white'
				: 'text-white/60 hover:text-white'}">Featured projects</button
		>
		...
		<div class="flex flex-col gap-4 pl-4">
			{#each domains as domain (domain)}
				<button
					...
					class="text-left text-base {activeDomain === domain
						? 'font-semibold text-white'
						: 'text-white/60 hover:text-white'}">{domain}</button
				>
			{/each}
		</div>
	</div>
	...
	<div class="flex grow flex-col gap-6">
		<!-- Project 1 -->
		<ProjectCard {...autonomousDataAgent} />

		<!-- Project 2 -->
		<ProjectCard {...devPortalX} />

		<!-- Project 3 -->
		<ProjectCard {...llmFineTuningBiasOptimization} />

		<!-- Project 4 -->
		<ProjectCard {...distributedHealthcareNetwork} />
